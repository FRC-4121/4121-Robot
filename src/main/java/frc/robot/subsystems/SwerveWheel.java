...
public class SwerveWheel extends SubsystemBase {
  ...
  private static final double angleDeadband = 0.001; // Deadband for the angle motor. Values smaller than this are
                                                     // rounded to 0.
  private static final double driveDeadband = 0.001; // Deadband for the drive motor. This works in the same way as the
                                                     // angle one.
  ...
  public SwerveWheel(WheelConfig config) {
    ...
    }
    {
      StatusCode code = driveConfig.apply(
          new MotorOutputConfigs()
              ...
              .withDutyCycleNeutralDeadband(driveDeadband), // Values smaller than the drive deadband are rounded down
                                                            // to 0.
          configTimeout);
      if (code != StatusCode.OK) {
        DriverStation.reportError(String.format("Configuring output levels for drive motor failed with code: %s", code),
            false);
      }
    }
    ...
    }
    {
      StatusCode code = angleConfig.apply(
          new Slot0Configs()
              .withKP(config.angleKP)
              .withKI(config.angleKI)
              .withKD(config.angleKD),
          configTimeout);
      if (code != StatusCode.OK) {
    ...
    // swerveDriveMotor.setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0,
    // 10, kTimeoutMsDrive);
    // swerveDriveMotor.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic,
    // 10, kTimeoutMsDrive);
  }
